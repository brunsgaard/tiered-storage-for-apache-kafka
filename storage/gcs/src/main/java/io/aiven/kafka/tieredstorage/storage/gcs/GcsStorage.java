...
public class GcsStorage implements StorageBackend {
    ...
    private <T> T runBounded(final Callable<T> action, final String description)
        ...
        final Future<T> future = operationExecutor.submit(() -> {
            ...
            try {
                // Check inside the try so the finally always runs: if the timeout/interrupt handler
                // already fired (it set cancelled and may have added this thread to the tracker's
                // cancelledWorkers via cancelWorker), we must still clear() on the way out — otherwise
                // this pooled thread is returned poisoned and the interceptor pre-aborts the next
                // healthy request that reuses it.
                if (cancelled.get()) {
